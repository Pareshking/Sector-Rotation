...
page_header(
    "Exposure Universe",
    "Themes",
    "Thematic rotation under the same hard boundary: authoritative history first, signal second, rank third.",
)

decisions = load_decisions()
frame = decisions[decisions.category == "thematic"] if not decisions.empty else decisions
if frame.empty:
    st.info("No theme observations are available.")
    st.stop()
...
kpi_strip(
    [
        ("Themes", counts["total"], "in the universe", ""),
...
st.plotly_chart(returns_heatmap(eligible, limit=21), width="stretch", config=CHART_CONFIG)
...
if signals.empty:
    note("No theme currently satisfies either the BUY or the REDUCE / EXIT rule.")
else:
    signal_rows(signals, rs=rs)

section("Full theme ranking", "Click any column header to sort")
...
ranked_table(eligible, rs=rs, sort_by=period or "Composite", key="themes_table")

unavailable = frame[~frame.decision_eligible]
if not unavailable.empty:
    with st.expander(f"Unavailable theme histories · {len(unavailable)}", expanded=False):
