...
page_header(
    "Exposure Universe",
    "Sectors",
    "Sector rotation against Nifty 50. Only authoritative decision-grade histories can produce BUY or REDUCE / EXIT.",
)
...
section("BUY")
render_decision_cards(buy, limit=20, empty_text="No sector currently satisfies the complete BUY rule.")

section("REDUCE / EXIT")
render_decision_cards(sell, limit=20, empty_text="No sector currently satisfies the REDUCE / EXIT rule.")

section("WATCH")
render_decision_cards(watch, limit=20, empty_text="No decision-grade sector is currently on WATCH.")

section("Sector ranking")
render_rank_list(actionable, limit=30)

with st.expander("Unavailable sector histories", expanded=False):
    unavailable = frame[~frame.decision_eligible]
    if unavailable.empty:
        st.info("No sector histories are currently excluded.")
    else:
