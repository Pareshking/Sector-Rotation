...
def build_live(registry):
    ...
    health = {
        ...
        "etf_skipped_symbols": [k for k in etf_keys if k not in etf_history or etf_history[k].dropna().size < 20],
...
def run(mode):
    ...
    for e in registry.all():
        ...
        rank_row = rankings.loc[e.id]
        summary_rows.append({
            "exposure_id": e.id,
            "exposure": e.name,
            "category": e.category.value,
            "benchmark": e.benchmark,
            "resolved_official_index_name": health.get("resolved_official_index_names", {}).get(e.id, e.benchmark),
            "data_source": health.get("source_by_canonical_exposure", {}).get(e.id, "unknown"),
