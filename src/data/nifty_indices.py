...
def _make_nifty_session(session=None, timeout=DEFAULT_TIMEOUT):
    """Warm a NiftyIndices session before POSTing to its ASP.NET endpoints."""
...
def _post_json(url, payload, timeout=DEFAULT_TIMEOUT, session=None):
    try:
        response = _make_nifty_session(session, timeout=timeout).post(url, headers=HEADERS, json=payload, timeout=timeout)
    ...
    for frame in frames:
        ...
        for row in frame.to_dict(orient="records"):
            ...
            if key and key not in seen:
                rows.append({"name": _canonical_name(name), "category": str(row.get("category") or "")})
                seen.add(key)
...
def _discover_index_catalogue_uncached(timeout=DEFAULT_TIMEOUT):
    ...
    for subtype in dict.fromkeys(subtypes):
        for row in _post_json(INDEX_CATALOGUE_ENDPOINT, {"cinfo": {"indextype": subtype, "indexgroup": "Equity"}}, timeout=timeout):
            ...
            if value:
                records.append({"name": _canonical_name(value), "category": subtype})
...
def resolve_catalogue_name(name, catalogue=None):
    ...
    normalized = {normalize_index_name(c): c for c in names}
    if requested_norm in normalized:
        return normalized[requested_norm]
    for candidate in INDEX_NAME_ALTERNATES.get(key, ()):
        if normalize_index_name(candidate) in normalized:
            return normalized[normalize_index_name(candidate)]
    scored = []
...
def resolve_index_names(name, catalogue=None):
    ...
    resolved = resolve_catalogue_name(name, catalogue)
    if resolved:
...
def _request_endpoint(endpoint_candidates, name, start, end, timeout=DEFAULT_TIMEOUT, session=None):
    ...
    s = _make_nifty_session(session, timeout=timeout)
    for endpoint in endpoint_candidates:
        try:
            response = s.post(endpoint, headers=HEADERS, json=payload, timeout=timeout)
            response.raise_for_status()
    ...
    for index_type in resolve_index_names(name, catalogue_frame):
        try:
            ...
            if len(series.dropna()) >= MIN_OBSERVATIONS:
                series.attrs.update(source="nse_api", resolved_name=index_type)
                return series
        except Exception:
            continue
    return pd.Series(dtype="float64", name=name)
...
def fetch_nse_api_indices(names, start, end, workers=4):
    ...
    def one(item):
        eid, name = item
        return eid, fetch_nse_api_index_history(name, start, end, catalogue=catalogue)
...
def _yahoo_candidates(exposure_id, index_name):
    symbol = VALID_YAHOO_INDEX_SYMBOLS.get(exposure_id)
...
def fetch_missing_indices(
    ...
    resolved_by_exposure = dict(frame.attrs.get("resolved_name_by_exposure", {}))
    catalogue = discover_index_catalogue()
    ...
    if missing:
        def one(item):
            eid, name = item
            return eid, fetch_nifty_index_history(name, years=years, start=start, end=date.today(), retries=2, catalogue=catalogue)

        with ThreadPoolExecutor(max_workers=min(5, len(missing))) as ex:
            futures = [ex.submit(one, item) for item in missing.items()]
            for fut in as_completed(futures):
                try:
                    eid, series = fut.result()
                except Exception:
                    continue
                if len(series.dropna()) >= MIN_OBSERVATIONS:
                    frame = frame.drop(columns=[eid], errors="ignore").join(series.rename(eid), how="outer")
                    source_by_exposure[eid] = series.attrs.get("source", "niftyindices")
                    resolved_by_exposure[eid] = series.attrs.get("resolved_name", mapping[eid])
    ...
    if missing:
        ...
        for eid in list(missing):
            if eid in api_frame and len(api_frame[eid].dropna()) >= MIN_OBSERVATIONS:
                frame = frame.drop(columns=[eid], errors="ignore").join(api_frame[eid].rename(eid), how="outer")
                source_by_exposure[eid] = "nse_api"
                resolved_by_exposure[eid] = mapping[eid]
                missing.pop(eid, None)
    ...
    if missing:
        ...
        for eid in list(missing):
            ...
            if matched is not None and len(archive[matched].dropna()) >= MIN_OBSERVATIONS:
                frame = frame.drop(columns=[eid], errors="ignore").join(archive[matched].rename(eid), how="outer")
    ...
    if missing:
        ...
        for eid in yahoo_frame.columns:
            frame = frame.drop(columns=[eid], errors="ignore").join(yahoo_frame[eid].rename(eid), how="outer")
            source_by_exposure[eid] = yahoo_sources.get(eid, "yahoo")
            resolved_by_exposure[eid] = mapping[eid]
            missing.pop(eid, None)
    ...
    if missing:
        ...
        for eid in seed_frame.columns:
            frame = frame.drop(columns=[eid], errors="ignore").join(seed_frame[eid].rename(eid), how="outer")
            source_by_exposure[eid] = seed_sources[eid]
            resolved_by_exposure[eid] = mapping[eid]
            missing.pop(eid, None)
