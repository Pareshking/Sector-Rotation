...
def _post_json(url: str, payload: object, timeout=DEFAULT_TIMEOUT, session=None) -> list[object]:
...
def _merge_catalogues(*frames: pd.DataFrame) -> pd.DataFrame:
    ...
    for frame in frames:
        ...
        for row in frame.to_dict(orient="records"):
            ...
            canonical, key = _canonical_name(name), normalize_index_name(name)
            if key and key not in seen:
                rows.append({"name": canonical, "category": str(row.get("category") or "")}); seen.add(key)
    return pd.DataFrame(rows, columns=["name", "category"])


def _discover_index_catalogue_uncached(timeout=DEFAULT_TIMEOUT) -> pd.DataFrame:
    subtypes = []
...
@lru_cache(maxsize=8)
def _discover_index_catalogue_memory(timeout):
    return _discover_index_catalogue_uncached(timeout)


def discover_index_catalogue(force_refresh=False, cache_seconds=CATALOGUE_CACHE_SECONDS, timeout=DEFAULT_TIMEOUT) -> pd.DataFrame:
    ...
    if not frame.empty:
        try:
            ...
            write_json_cache(frame.to_dict(orient="records"), CATALOGUE_CACHE_FILE)
        except Exception: pass
    return frame
...
def resolve_catalogue_name(name: str, catalogue: pd.DataFrame | None = None) -> str | None:
    ...
    names = [str(v) for v in catalogue["name"].dropna().tolist()]
    ...
    for candidate in names:
        cn = normalize_index_name(candidate)
        overlap = len(set(requested_norm.split()) & set(cn.split())) / max(len(set(requested_norm.split()) | set(cn.split())), 1)
        scored.append((0.65 * overlap + 0.35 * SequenceMatcher(None, requested_norm, cn).ratio(), candidate))
    if scored:
...
def resolve_index_names(name: str, catalogue: pd.DataFrame | None = None) -> list[str]:
    key = str(name).strip().casefold(); candidates = list(INDEX_NAME_ALTERNATES.get(key, ()))
...
def _request_endpoint(endpoint_candidates: Sequence[str], name: str, start: date, end: date, timeout=DEFAULT_TIMEOUT, session=None):
    payload = {"cinfo": "{'name':'%s','startDate':'%s','endDate':'%s','indexName':'%s'}" % (name, start.strftime("%d-%b-%Y"), end.strftime("%d-%b-%Y"), name)}
    s = _make_session(session)
    for endpoint in endpoint_candidates:
        try:
            response = s.post(endpoint, headers=HEADERS, json=payload, timeout=timeout); response.raise_for_status()
...
def _rows_to_series(name: str, rows: list[dict[str, object]], tri=False) -> pd.Series:
...
def _request_history(name, start, end, timeout=DEFAULT_TIMEOUT, tri=True):
...
def fetch_nifty_index_history(name, years=5, start=None, end=None, retries=1, timeout=DEFAULT_TIMEOUT, catalogue=None):
    end_date, start_date = end or date.today(), start or (end or date.today()) - timedelta(days=365 * years + 10)
    catalogue_frame = catalogue if catalogue is not None else discover_index_catalogue(timeout=timeout)
    for _ in range(max(retries, 1)):
        for candidate in resolve_index_names(name, catalogue=catalogue_frame):
            tri = _request_history(candidate, start_date, end_date, timeout=timeout, tri=True)
            if len(tri.dropna()) >= MIN_OBSERVATIONS:
                tri.attrs.update(source="niftyindices_tri", resolved_name=candidate); return tri
            pr = _request_history(candidate, start_date, end_date, timeout=timeout, tri=False)
            if len(pr.dropna()) >= MIN_OBSERVATIONS:
                pr.attrs.update(source="niftyindices_pr", resolved_name=candidate); return pr
...
def fetch_nse_api_index_history(name, start, end, timeout=DEFAULT_TIMEOUT, catalogue=None):
    ...
    for index_type in resolve_index_names(name, catalogue=catalogue_frame):
        try:
            ...
            payload = response.json(); data = payload.get("data", {}) if isinstance(payload, dict) else {}; rows = data.get("indexCloseOnlineRecords", []) if isinstance(data, dict) else []
            series = _rows_to_series(name, rows)
            if len(series.dropna()) >= MIN_OBSERVATIONS:
                series.attrs.update(source="nse_api", resolved_name=index_type); return series
        except Exception: continue
    return pd.Series(dtype="float64", name=name)
...
def fetch_nse_api_indices(names, start, end, workers=4):
    ...
    catalogue = discover_index_catalogue(); results, unresolved = {}, {}
    def one(item):
    ...
    unresolved = {eid: name for eid, name in names.items() if eid not in results}
...
def _proxy_from_etfs(missing, etf_histories):
    ...
    for exposure_id in missing:
        for key in ETF_PROXY_KEYS.get(exposure_id, ()):
            if key in etf_histories and len(etf_histories[key].dropna()) >= MIN_OBSERVATIONS:
                result[exposure_id] = etf_histories[key].dropna().rename(exposure_id); source[exposure_id] = "etf_proxy"; break
    return pd.DataFrame(result).sort_index(), source
...
def fetch_missing_indices(names: Mapping[str, str] | Iterable[tuple[str, str]], existing=None, years=5, etf_histories=None):
    mapping, frame = dict(names), (existing.copy() if existing is not None else pd.DataFrame())
    source_by_exposure, resolved_by_exposure = dict(frame.attrs.get("source_by_exposure", {})), dict(frame.attrs.get("resolved_name_by_exposure", {}))
    catalogue = discover_index_catalogue()
    missing = {eid: name for eid, name in mapping.items() if eid not in frame.columns or len(frame[eid].dropna()) < MIN_OBSERVATIONS}
    ...
    if missing:
        start = date.today() - timedelta(days=365*years+10); api_frame, _ = fetch_nse_api_indices(missing, start, date.today(), workers=5)
    ...
    frame.attrs["unresolved_exposures"] = sorted(missing)
