...
def _parse_history(text: str) -> pd.DataFrame:
    ...
    if not frame.empty:
        ...
        frame["date"] = pd.to_datetime(raw_dates, format="%d-%b-%Y", errors="coerce").fillna(
            pd.to_datetime(raw_dates, format="%d-%m-%Y", errors="coerce")
        )
...
def find_scheme_codes(current_nav: pd.DataFrame, scheme_names: Iterable[str]) -> dict[str, str]:
    ...
    if current_nav.empty or "scheme_name" not in current_nav.columns:
        return {}
    normalized = current_nav.assign(
        _name=current_nav["scheme_name"].astype(str).str.upper().str.replace(r"[^A-Z0-9]", "", regex=True)
    )
    result: dict[str, str] = {}
